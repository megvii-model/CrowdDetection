...
def set_cpu_nms_body_with_head(body_dets, head_dets, thresh):
    """Pure Python NMS baseline."""
    def _overlap(det_boxes, basement, others):
        eps = 1e-8
        x1_basement, y1_basement, x2_basement, y2_basement \
    ...
    scores = body_dets[:, 4]
    order = np.argsort(-scores)
    body_dets = body_dets[order]
    head_dets = head_dets[order]

    numbers = body_dets[:, 5]
    keep = np.ones(len(body_dets)) == 1
    ruler = np.arange(len(body_dets))
    while ruler.size>0:
        basement = ruler[0]
        ruler=ruler[1:]
        num = numbers[basement]
        # calculate the body overlap
        overlap_body = _overlap(body_dets[:, :4], basement, ruler)
        overlap_head = _overlap(head_dets[:, :4], basement, ruler)
        overlap = np.maximum(overlap_body, overlap_head)
        indices = np.where(overlap > thresh)[0]
        loc = np.where(numbers[ruler][indices] == num)[0]
        # the mask won't change in the step
        mask = keep[ruler[indices][loc]]#.copy()
