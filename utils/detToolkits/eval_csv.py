...
def main():
    ...
    parser.add_argument('--imgroot', dest='imgroot', default='', \
                        help='the path of the root dir of images')
    ...
    gtpath, dtpaths, imgroot = args.gt, args.dt, args.imgroot
    ...
    for (dtpath, showThr, filterThr) in zip(dtpaths, showThres, filterThres):
        ...
        db = EvalDB(dbName, imgroot, gtpath, dtpath, showThr)
    ...
    # evaluator.eval_AP_detail(iouThres=0.5, csvname=args.csv)
    evaluator.eval_AP_detail(iouThres=0.5)
    evaluator.eval_MR(iouThres=0.5)
    # print("AP is {}, MR is {}".format(AP,MR))
