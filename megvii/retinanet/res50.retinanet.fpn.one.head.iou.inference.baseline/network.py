...
class Network(nn.Module):
    ...
    def forward(self, image, im_info, gt_boxes=None):
        ...
        pred_cls_list, pred_reg_list, rpn_iou_list = self.R_Head(fpn_fms)
        # release the useless data
        if self.training:
            loss_dict = self.R_Criteria(
                pred_cls_list, pred_reg_list, anchors_list, rpn_iou_list, gt_boxes, im_info)
            return loss_dict
        else:
    ...
    def forward_inference(self, anchors_list, pred_cls_list,
        ...
        anchors = all_anchors.unsqueeze(1).repeat(1, 1, 1).view(-1, all_anchors.shape[1])
...
class RetinaNetCriteriaV2(nn.Module):
    ...
    def anchor_iou_target_opr(self, boxes, im_info, all_anchors,
        ...
        for i in range(n):
            ...
            index = torch.argmax(overlaps, dim = 1)
            a = torch.gather(overlaps, 1, index.unsqueeze(1))
            res.append(a.unsqueeze(0))

        result = torch.cat(res, 0)
        return result

    def forward(self, pred_cls_list, pred_reg_list,
...
>>>>>>> 126a572b4b37b562f2a6f27ee126688c37d13cf9
    ...
    def forward(self, features):
        ...
        cls_prob_list, pred_bbox_list, rpn_iou_prob_list = [], [], []
        ...
        pred_cls_list = [
            _.permute(0, 2, 3, 1).reshape(_.shape[0], -1, config.num_classes-1)
            for _ in cls_prob_list]
        pred_reg_list = [
            _.permute(0, 2, 3, 1).reshape(_.shape[0], -1, 4)
            for _ in pred_bbox_list]
        rpn_iou_list = [
            _.permute(0, 2, 3, 1).reshape(_.shape[0], -1, config.num_classes-1)
            for _ in rpn_iou_prob_list]

        return pred_cls_list, pred_reg_list, rpn_iou_list
