...
class CascadeRCNN(nn.Module):
    ...
    def __init__(self, iou_thresh, nheads, stage):
        ...
        self.is_refine = True if self.nheads > 1 else False
        ...
        if self.is_refine:
            ...
            self.fc3 = nn.Linear(1054, 1024)
            self.q = nn.Linear(1024, 5 * nheads)
            self.r = nn.Linear(1024, 5 * nheads)
    ...
    def _init_weights(self):
        ...
        if self.is_refine:
            for l in [self.fc3, self.q, self.r]:
                nn.init.normal_(l.weight, std=0.01)
                nn.init.constant_(l.bias, 0)

    def refinement(self, fc2, prob):

        assert self.is_refine
        
        n = prob.shape[0]
        prob = prob.view(n * self.n, -1)
        offsets, scores = prob[:, :-self.n], prob[:, -self.n:]
        offsets = offsets.reshape(-1, self.n, 4)
        cls_scores = torch.softmax(scores, dim=1)
        pred_boxes = torch.cat([offsets, cls_scores.unsqueeze(2)], dim=2)[:, 1]
        rboxes = pred_boxes.unsqueeze(1).repeat(1, 6, 1).reshape(offsets.shape[0], -1)
        fc2 = fc2.unsqueeze(1).repeat(1, self.nheads, 1).reshape(-1, fc2.shape[1])

        fc3 = torch.cat([fc2, rboxes], dim=1)
        fc3 = self.relu(self.fc3(fc3))

        _, c= fc3.shape
        fc3 = fc3.reshape(-1, self.nheads, c).permute(1, 0, 2)

        q, r = self.q(fc3[0]), self.r(fc3[1])
        prob = torch.cat([q, r], dim = 1)
        return prob
    ...
    def forward(self, fpn_fms, rois, gtboxes=None, im_info = None):
        ...
        if self.is_refine:
            final_prob = self.refinement(fc2, prob)
        ...
        if self.training:
            ...
            if self.is_refine:
                final_loss = self.compute_gemini_loss_opr(final_prob, bbox_targets, labels)

                value = sum([v for (_, v) in final_loss.items()])
                refine_loss = {self.name + '_refine_loss': value}
            ...
            if self.is_refine:
                pred_boxes = self.recover_pred_boxes(rcnn_rois, final_prob, self.nheads)
            return pred_boxes
